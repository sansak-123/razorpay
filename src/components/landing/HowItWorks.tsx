const STEPS = [
  ...
  { n: "02", title: "Classify", detail: "Deterministic rules resolve what they can; AI reasons only over what's genuinely ambiguous." },
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 md:gap-6">
        {STEPS.map((step) => (
          <div key={step.n} className="md:border-l md:border-white/10 md:pl-5">
            <div className="font-mono text-[14.5px] text-blue mb-2">{step.n}</div>
            <div
              className="text-landing-white font-semibold text-[19px] mb-1.5"
              style={{ fontFamily: "var(--font-space-grotesk)" }}
            >
              {step.title}
            </div>
            <p className="text-[15.5px] text-landing-text-dim leading-relaxed">{step.detail}</p>
          </div>
        ))}
      </div>
    </section>
