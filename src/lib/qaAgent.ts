...
function retrieveContext(
  ...
  if (/\b(match rate|matched|total orders|how many orders|overview|summary)\b/i.test(question)) {
    ...
    return [
      ...
      `  ${s.ai_reasoned_count} of those exceptions were reasoned live by AI; the rest were confident rule matches.`,
...
export async function answerSettlementQuestion(
  ...
  try {
    ...
    console.warn("answerSettlementQuestion: LLM call failed, falling back to raw context", err);
    return {
